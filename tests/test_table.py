...
def test_get_batch(TestModel, TestModel_entries, dynamo_local):
    items = TestModel.get_batch(
        keys=({"foo": "first", "bar": "one"}, {"foo": "first", "bar": "three"}),
        attrs="bar",
    )
...
def test_get_batch_invalid_field(TestModel):
    ...
    with pytest.raises(InvalidSchemaField):
        list(TestModel.get_batch(keys=({"invalid": "nope"},)))
...
def test_query_filter(TestModel, TestModel_entries, dynamo_local):
    ...
    results = list(
        TestModel.query(Q(count__gt=222) | Q(count__lt=222), ~Q(count=111), foo="first")
    )
    assert len(results) == 1
...
def test_update_invalid_fields(TestModel, TestModel_entries, dynamo_local):
    ...
    with pytest.raises(ConditionFailed):
        TestModel.update_item(
            ...
            # things to update
            baz="foo",
            conditions=dict(unknown_attr="foo"),
        )


def test_schema_field_removed_update_return_all(
...
def test_scan_iterator_recursive(
    TestModel, TestModel_entries_xlarge, dynamo_local, mocker
):
    try:
        mocker.spy(TestModel.Table.__class__, "scan")
    except TypeError:
...
def test_specific_attributes(TestModel, TestModel_entries, dynamo_local):
    results = list(
        TestModel.query(foo="first").specific_attributes(
            ["foo", "bar", "count", "child.sub"]
        )
    )
...
def test_config():
    class TestTable(DynamoTable3):
        resource_kwargs = {"region_name": "us-west-2", "config": {"connect_timeout": 1}}
    ...
    class BadConfigTable(DynamoTable3):
        resource_kwargs = {"config": {"foo": True}}
